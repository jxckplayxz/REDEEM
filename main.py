# app.py — VIXN — FINAL CLEAN & SMALLER VERSION (Name fixed + perfect size)
...
def add_product():
    try:
        ...
        new_prod = {
            "id": next_id(), "name": name, "price": round(price, 2),
...
# TEMPLATES
LOGIN_HTML = """<!doctype html><html><head><title>VIXN • Admin</title>
<style>body{background:#09090b;color:#e6eef8;display:grid;place-items:center;height:100vh;margin:0;font-family:system-ui}
.box{background:#0f172a;padding:40px;border-radius:16px;width:360px}
input,button{padding:12px;margin:8px 0;width:100%;border-radius:8px;border:none;background:#1e293b;color:white}
button{background:#3b82f6;cursor:pointer;font-weight:600}
</style></head><body><div class="box"><h2>VIXN Admin</h2>
<form method=post>
...
ADMIN_HTML = """<!doctype html><html><head><title>VIXN • Admin</title>
<style>
...
<tr><td>{{p.timestamp[:19].replace("T"," ")}}</td><td>{{p.email}}</td><td>${{p.total}}</td></tr>{% endfor %}</table></div>

<script>
document.getElementById("f").onsubmit=e=>{e.preventDefault();fetch("/api/add_product",{method:"POST",body:new FormData(e.target)}).then(()=>location.reload())}
</script>
</body></html>"""

# CLEAN, SMALLER, PERFECT VIXN HOME
...
function update(){
    ...
    c.forEach(item=>{
        ...
        items.innerHTML+=`<div class="item"><img src="${esc(item.image)}">
            <div style="flex:1"><h3 style="margin:0;font-size:16px">${esc(item.name)}</h3>
            <p style="margin:4px 0 0;font-size:14px;color:var(--muted)"> \]{item.price.toFixed(2)} × ${item.qty}</p></div></div>`;
    });
