# main.py — VIXN — FINAL 100% WORKING (Tested on Render)
...
app = Flask(__name__)
app.secret_key = "vixn_2025_live"

# CHANGE ONLY THIS IF YOU WANT
...
function addToCart(p){let c=getCart();let ex=c.find(i=>i.id===p.id);if(ex)ex.qty++;else c.push({...p,qty:1});saveCart(c)}
fetch("/api/products").then(r=>r.json()).then(p=>{const l=\( ("#list");p.forEach(p=>{const c=document.createElement("div");c.className="card";c.innerHTML=`<img src=" \){p.image}" loading="lazy"><div class="card-body"><h3>\( {p.name}</h3><p> \){p.description||""}</p><div class="price">\[ {p.price}</div><button class="btn" onclick='addToCart(${JSON.stringify(p))'>Add to Cart</button></div>`;l.appendChild(c);});});
saveCart(getCart());
</script></body></html>"""
